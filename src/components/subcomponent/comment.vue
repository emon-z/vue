  ...
  methods: {
    getcommentlist() {
      this.$http.get("/src/data/comment.json").then(result => {
        if (result.body.status == 200) {
          ...
          this.comments=this.comments.concat(JSON.parse(localStorage.getItem('cmts')||'[]'))
        } else {
    ...
    add() {
      if (this.inner == "") {
        ...
        var newdata={username:"孤芒",add_time:"2019-01-23",content:this.inner}
        var list=JSON.parse(localStorage.getItem('cmts')||'[]')
        var newlist=[]
        newlist.push(newdata)
        list=list.concat(newlist)
        localStorage.setItem('cmts',window.JSON.stringify(list))
